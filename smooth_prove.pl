...
check(Env, D, Prop, verif) :- is_env(Env), is_prop(Prop), check_verif(Env, D, Prop), is_verif_deriv(D).
check(Env, D, Prop, use) :- is_env(Env), is_prop(Prop), check_use(Env, D, Prop), is_use_deriv(D).
check(Env, D, Prop, prop) :- is_env(Env), is_prop(Prop), check_prop(Env, D, Prop), is_prop_deriv(D).
check(Env, D, Set, set) :- is_env(Env), is_set(Set), check_set(Env, D, Set), is_set_deriv(D).
